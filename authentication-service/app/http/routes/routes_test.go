...
func TestPOSTTokenAuth(t *testing.T) {
	...
	selectByIDRows := sqlmock.NewRows([]string{"id", "user_name", "createdAt", "password", "user_mail"}).AddRow(user.ID, user.Name, timeNow, hash, user.Email)
	mock.ExpectQuery("SELECT (.+) FROM users WHERE").WithArgs(user.Name).WillReturnRows(selectByIDRows)
